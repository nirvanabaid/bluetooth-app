...
class casnode extends StatefulWidget {
  final BluetoothDevice server;

  const casnode({required this.server});

  @override
  _casnode createState() => new _casnode();
}
...
class _casnode extends State<casnode> {
            ...
            children: <Widget>[
              ...
              Padding(
                ...
                child: ElevatedButton(
                  onPressed: isConnected
                      ? () async {
                          await _sendMessage("id");
                          showSnackBar(context, "SENT SUCCESSFULLY", Colors.green);
                          await changeDetect();
                          i=no_of_messages_received;
                          _showSimpleDialog(latestReadData, "ID");
                  }
                      : null,
                  child: Center(
                    child: Text(
              ...
              Padding(
                ...
                child: ElevatedButton(
                  onPressed: isConnected
                      ? () async {
                    await _sendMessage("OTA");
                    showSnackBar(context, "SENT SUCCESSFULLY", Colors.green);
                    await changeDetect();
                    i=no_of_messages_received;
                    _showSimpleDialog(latestReadData, "OTA");
                  }
                      : null,
              ...
              Padding(
                ...
                child: ElevatedButton(
                  onPressed: isConnected
                      ? () async{
                    await _sendMessage("sha");
                    showSnackBar(context, "SENT SUCCESSFULLY", Colors.green);
                    await changeDetect();
                    i=no_of_messages_received;
                    _showSimpleDialog(latestReadData, "SHA");
                  }
                      : null,
                  child: Center(child: Text("SHA", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w500, fontSize: height*0.02),),),
                  style: ButtonStyle(
                    backgroundColor: MaterialStatePropertyAll(background),
                    shape: MaterialStatePropertyAll(RoundedRectangleBorder( borderRadius: BorderRadius.circular(9), side: BorderSide(width: 2, color: highlight))),
                    padding: MaterialStatePropertyAll(EdgeInsets.symmetric(vertical: height*0.016)),
  ...
  Future<void> _showFlatDialog(String a ) async{
    ...
    final imageList = ['assets/casnode.png'];
    var height=MediaQuery.of(context).size.height;
    var width=MediaQuery.of(context).size.width;
    await showDialog<void>(
